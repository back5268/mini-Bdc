...
export const projectMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    if (req.userInfo?.type !== 'admin') {
      const projects = req.userInfo.department?.projects;
      if (!projects.includes(project)) return res.status(401).json({ status: false, mess: 'Bạn không có quyền quản lý dự án này!' });
    }
